...
void ff_tx_task ()
{
  ...
  while (1) {
	...
	// Enable link acks on p2p if the flag is set and not broadcast address
        if ((p2p_pkt.ctrl_flags & LINK_ACK) != 0 && p2p_pkt.dst_subnet_mac[0]!=255) {
...
void sampl_config ()
{
  ...
  SAMPL_RX_TASK.period.secs = 0;
  SAMPL_RX_TASK.period.nano_secs = 1;
  SAMPL_RX_TASK.cpu_reserve.secs = 0;
  ...
  SAMPL_TX_TASK.cpu_reserve.secs = 0;
