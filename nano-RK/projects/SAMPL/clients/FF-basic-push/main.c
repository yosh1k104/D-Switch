  ...
  if(my_sigs & SIG(async_sig))
	{
 	...
 	async_sensor_pkt[0]=my_mac;
 	async_sensor_pkt[1]=TRAN_BINARY_SENSOR;
 	async_sensor_pkt[2]=1;  // tran_pkt packet len
 	async_sensor_pkt[3]=(1<<7) | BINARY_SENSOR_LEAK;

	for(i=0; i<2; i++ )
		{
 		// push_p2p_pkt( payload, size, pkt_type, dst_mac );
 		push_p2p_pkt( async_sensor_pkt,4,TRANSDUCER_PKT, 0x0 );
