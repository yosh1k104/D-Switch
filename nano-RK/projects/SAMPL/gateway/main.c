...
//    for (i = 0; i < len; i++) printf ("%d ", rx_buf[i]); printf ("]\r\n");
	...
	gw_pkt.subnet_mac[0]=p2p_pkt.src_subnet_mac[0];
	gw_pkt.subnet_mac[1]=p2p_pkt.src_subnet_mac[1];
	gw_pkt.subnet_mac[2]=p2p_pkt.src_subnet_mac[2];
	gw_pkt.dst_subnet_mac[0]=p2p_pkt.dst_subnet_mac[0];
	gw_pkt.dst_subnet_mac[1]=p2p_pkt.dst_subnet_mac[1];
	gw_pkt.dst_subnet_mac[2]=p2p_pkt.dst_subnet_mac[2];
	...
	if(us_pkt.subnet_mac[2]==my_subnet_mac[2] &&
...
void tx_task ()
{
  uint8_t j, i, val, cnt,error,CTR_buf[4],checksum;
  int8_t len;
...
error=1;
     ...
     if(len!=NRK_ERROR && len>10 && len<120) { 
     		printf( "len = %u\r\n", len );
	     	...
	     	nrk_kprintf( PSTR("Serial Packet Checksum passed\r\n")); 
		error=0; 
		}
     else {
	     nrk_kprintf( PSTR("Serial Packet Checksum Failed!\r\n" )); 
	     printf( "Got %d not %d\r\n", tx_buf[len-1], checksum );
	     error=1; 
	  }
...
if(tx_buf[CTRL_FLAGS] & DS_MASK != 0)
{
  ...
  //  tx_buf[PKT_TYPE]=SENSOR_LONG_PKT;
   // tx_buf[CTRL_FLAGS]=   TREE_FILTER | LINK_ACK |  ENCRYPT |  DS_MASK  |  DEBUG;
     tx_buf[CTRL_FLAGS]=   LINK_ACK |  ENCRYPT |  DS_MASK  |  DEBUG_FLAG;
   // tx_buf[CTRL_FLAGS]=     ENCRYPT |  DS_MASK  |  LED_FLOOD;
    //tx_buf[CTRL_FLAGS]=    TREE_FILTER |  ENCRYPT |  DS_MASK  |  LED_FLOOD;
    //tx_buf[CTRL_FLAGS]=   LINK_ACK | MAC_FILTER | TREE_FILTER | ENCRYPT |  DS_MASK  |  DEBUG;
   ...
   if(error==0)
	{
		...
		// Update counter, or disable encryption based on encryption flag
		...
		// re-enable encryption if it was off
		if((tx_buf[CTRL_FLAGS] & ENCRYPT)== 0 ) bmac_encryption_enable();
