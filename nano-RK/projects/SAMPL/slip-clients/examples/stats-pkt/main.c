...
float avg_current;
float cpu_avg_current;
float radio_avg_current;
float radio_checks;
...
//       downstream_packet_add_mac_filter( &ds_pkt, 2 ); 
   ...
   while(reply_timeout>time(NULL))
   	{
		...
		if(echo==0)
			...
			if(ef==0) printf( "Gateway packet returned correctly.\n" );
			//else exit(0);
			echo=1;
			} else
    		gw_pkt.buf=rx_buf;
   ...
   if(echo==0) { printf( "Error, no serial reply from gateway!\n" );  goto retry; }
