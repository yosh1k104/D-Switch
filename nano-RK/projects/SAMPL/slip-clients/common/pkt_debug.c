...
unpack_gateway_packet(gw_pkt );
	...
	case STATS_PKT:
		for(i=0; i<gw_pkt->num_msgs; i++ )
		{
		  ...
		  printf( "  Radio current: %f\n", radio_avg_current );
		  printf( "  CPU current: %f\n", cpu_avg_current );
		  printf( "  Avg current: %f\n", avg_current );
		  printf( "  Estimated Life: %f days\n", (BATTERY_CAPACITY/avg_current)/24 );
		}
		break;
	...
	case TRANSDUCER_PKT:
                ...
                {
                case TRAN_POWER_PKT:
                        if(tran_msg.payload[0]==SENSE_PKT)
                        {
