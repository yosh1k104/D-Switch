...
*/
    ...
    if (p2p_pkt.payload_len != 0 && strlen (tmp_msg) != 0) {
      ...
      tx_q_add (p2p_pkt.buf, p2p_pkt.buf_len,0);
    }
  }
  in_body = 0;
...
*/
  ...
  if (XML_Parse (p, buf, strlen (buf), 1) == XML_STATUS_ERROR) {
    sprintf (global_error_msg, "XML Parse Error at line %u:\n%s\n",
...
// If the jid was not active, then open new connection 
  if (active == 0) {
    ...
    if (proxy_con[con].connection == NULL) {
      log_write ("Could not start xmpp proxy client 2");
      return -1;
    }
    strcpy (proxy_con[con].src_jid, src_jid);
    printf ("Adding %s as src jid with mac_addr = %d\n", src_jid, mac_addr);
